...
const VideoPlayer = ({ src, id }) => {
  ...
  const handleVolumeChange = (e) => {
    ...
    setState(prevState => ({ ...prevState, volume: newVolume, isMuted: newVolume === 0 }));
  };
  ...
  const handleLoadedMetadata = () => {
    setState(prevState => ({ ...prevState, duration: videoRef.current.duration, progress: 0 }));
  };
  ...
  const updateProgress = useCallback(() => {
    setState(prevState => ({ ...prevState, progress: videoRef.current.currentTime }));
  }, []);
    ...
    <div className="relative w-full bg-black rounded-lg overflow-hidden shadow-lg group">
      ...
      >
        <span className='p-4 rounded-full bg-gray-900/50'>
          {isPlaying ? <PauseIcon className="h-12 w-12 text-white" /> : <PlayIcon className="h-12 w-12 text-white" />}
        </span>
      </button>

      <div className={`absolute ${isPlaying ? 'group-hover:flex' : ''} hidden bottom-0 left-0 right-0 flex items-center justify-between gap-2 p-3 bg-gray-900 bg-opacity-75`}>
        ...
        <div className="slider-container w-full">
          <input
            ...
            style={{ '--min': 0, '--max': duration || 100, '--val': progress + 0.6 }}
          />
        </div>

        <div className="flex items-center space-x-2">
          <button onClick={handleMuteToggle} className="text-white p-1">
            {isMuted || volume === 0 ? <SpeakerXMarkIcon className="h-5 w-5 text-white" /> : <SpeakerWaveIcon className="h-5 w-5 text-white" />}
          </button>
          <input
            type="range"
            min="0"
