...
const Sidebar = ({
  ...
  const handleTabChange = (event, index) => {
    ...
    dispatch(
      setCurrentTab({
        name: `sidebar_${name}`,
        details: { selectedIndex: index, label: event.target.innerText },
      })
    );
  };
      ...
      <TabList className="p-4 md:p-6 xl:p-8 bg-[#FFFFFF] border border-[#d5bee6] rounded-2xl flex lg:flex-col lg:items-start flex-wrap gap-2 lg:gap-1 items-center">
        {sideBarItems?.[name].map((item, index) => (
          <Tab
            ...
            className={({ selected }) =>
              classNames(
                "flex gap-2 py-2 items-center outline-none",
                itemClassName,
                selected
                  ? "text-[#4361EE] font-bold"
                  : "text-[#17181C] font-bold"
              )
            }
            ...
            {({ selected }) => (
              <>
                ...
                {!showIconWithActive && item?.icon && (
                  <SvgIcon
                    name={item?.icon}
                    className={`${selected ? "stroke-[#4361EE]" : "stroke-[#17181C]"}`}
                  />
                )}
                <button
