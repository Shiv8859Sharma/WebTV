...
const InputField = (props) => {
  let { label = '', labelClass = '', type = 'text', name = '', defaultValue = '', placeholder = '', containerClass = '', inputClass = '', rightIcon = '', rightIconClass = '', leftIcon = '', leftIconClass = '', required = false, errorElement = 'span', errorMessageClassName = '', inputContainerClass = '', autoComplete = 'off' } = props
    ...
    <div className={`${name} w-full flex flex-col gap-2.5 ${containerClass}`}>
      <div className="input-container flex flex-col gap-2">
        ...
        <div className={`${name}_field px-4 py-3 border border-[#BBBDC8] rounded-full flex gap-2 ${inputContainerClass}`}>
          ...
          {
            rightIcon && (
              <div className={`w-[1.5rem] ${rightIconClass}`}>
                {rightIcon}
              </div>
            )
          }
