...
const ArticleCategory = ({ title, articles }) => {
  return (
    <div>
      <h2 className="text-2xl font-semibold border-b-2 border-gray-400 pb-4 mb-4">{title}</h2>
      {articles.map((article, index) => (
        <NavigatePage url={article?.link} key={index} className="mb-6 flex flex-col gap-3">
          <ArticleHeading
