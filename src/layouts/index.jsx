...
const layout = ({ userType }) => {
    ...
    return (
      <div className="flex items-center justify-center h-full w-[100vw]">
        <p className="text-center flex flex-col justify-center items-center gap-3">
          ...
          <img src={logo} loading='lazy' alt='logo' className="h-16 w-16 md:h-24 md:w-24 lg:h-32 lg:w-32" />
  ...
  const headerHeight = useMemo(() => (userType === '' ? 'h-[8rem] lg:h-[13rem]' : 'h-[9rem]'), [userType]);
  const contentHeight = useMemo(() => (userType === '' ? 'h-[calc(100vh-8rem)] lg:h-[calc(100vh-13rem)]' : 'h-[calc(100vh-9rem)]'), [userType]);

  // Determine sidebar visibility
  const showSidebar = useMemo(() => hasSideBar.includes(userType), [hasSideBar, userType]);
    ...
    <div className="h-full w-full overflow-hidden">
      <Suspense fallback={<InitialLoader />}>
        ...
        <div className={`${contentHeight} w-full min-h-40`}>
          <div ref={contentLayoutRef} className="h-full overflow-x-auto scrollbar">
